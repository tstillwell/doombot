...
def main(parser, args, parameter_server=None):
    ...
    params.eval_time = 100      # evaluation time (in seconds)
    ...
    if params.reload:
        ...
        # TS I updated this code to fix a problem with the reload state mismatching the training data
        state = network.module.state_dict()
        new_state = {}

        for key, val in reloaded.items():
            if key in state:
                if val.size() == state[key].size():
                    new_state[key] = val
        
        state.update(new_state)
        network.module.load_state_dict(state)
