...
class DQN(object):
    ...
    def register_loss(self, loss_history, loss_sc, loss_gf):
        loss_history['dqn_loss'].append(loss_sc.item())  # Use .item() to convert to a Python number
        loss_history['gf_loss'].append(loss_gf.item() if self.n_features else 0)  # Use .item() for scalar tensor
    ...
    def next_action(self, last_states, save_graph=False):
        ...
        if self.params.network_type == 'dqn_ff':
            ...
            # Check size and reshape for RNN network
        ...
        max_values, max_indices = scores.data.max(0)

        if max_indices.dim() == 0:
            action_id = max_indices.item()
        else:
            if max_indices.size(0) > 0:
                action_id = max_indices[0].item()
            else:
                raise RuntimeError("max_indices has no valid indices.")
