...
def setupGame():
    ...
    game = vzd.DoomGame()
    game.set_doom_scenario_path(os.path.join('./dm_config/', "zombie_deathmatch.wad"))
...
def decideNextMove(screen_buffer, navigating):
    ...
    b = cv2.split(screen_buf[:, :, 0])
    gray_image = cv2.cvtColor(screen_buffer, cv2.COLOR_BGR2GRAY)
    # cv2.imwrite('./gray_image.png', gray_image)
    depth = state.depth_buffer
    if (tickCount > 24 and navigating):
        return {'action' : 'findTargets', 'buffer' : b}
    if navigating:
        if (map and shouldRescan(depth)):
            map.clear()
...
def getAttackAngle(x_destination, resolution):
    chosen_destination = x_destination
    destination_midpoint = math.floor(chosen_destination)
    screen_midpoint = math.floor(resolution[1] / 2)
    x_delta = (destination_midpoint - screen_midpoint)
    y_delta = (resolution[0] - math.floor(resolution[0] / 2))
    attack_angle = math.atan2(x_delta , y_delta)
    attack_angle_degrees = math.degrees(attack_angle)
...
def scanForTargets(screen_buffer):
    brightest_idx = [0, 0]
    brightest_val = 0
    for i in range(len(screen_buffer[0]) - 90):
        for (idx, pixel) in enumerate(screen_buffer[0][i]):
            if pixel > brightest_val:
                brightest_val = pixel
                brightest_idx[0] = idx
                brightest_idx[1] = i
    return {"x": brightest_idx[0], "y": brightest_idx[1], "val": brightest_val}
...
if __name__ == "__main__":
    ...
    for i in range(episodes):
        ...
        while not game.is_episode_finished():
            ...
            if (next_move['action'] == 'findTargets'):
                targ = scanForTargets(next_move['buffer'])
                sleep(2)

                if (targ['val'] == 255):
                    angle = getAttackAngle(targ['x'],  screen_buf.shape)
                    actions[4][angle_index] = angle
        ...
        kill_reward = kills * 1000
        ammo = game.get_game_variable(vzd.GameVariable.AMMO2)
        ammo_reward = ammo * 16

        print("Kills:", kills)
        print("Ammo: ", ammo)
        print("Total reward:", game.get_total_reward() + kill_reward + ammo_reward)
        print("************************")
